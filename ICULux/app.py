...
def data():
     ...
     rows = rows.collect()
     cnrows = cnrows / len(distinct_rows)
     print("Lines: " + str(cnrows))
...
def data_stream():
     ...
     def inner(rows, dict):
          ...
          for i in rows[-int(600*(len(distinct_rows))):]:
               time.sleep(div)
               name = i[0]
               val1 = i[1]
               val2 = i[2]
               ...
               if (val3 != None and val2 != None):
                    dict[name] = list(filter(None, dict[name]))
                    if (len(dict[name]) < 60):

                       dict[name].append(val3)

                       dict[name] = [x for x in dict[name] if x is not None]
                    ...
                    print(name + " : " + str(val1) + "\t" + str(val2) + "\t" + str(val3))
                    ...
                    if ((int_or_float(val3) == 0) or (int_or_float(val2) == 0) or (int_or_float(val1)==0)):
                         condition = "[INACTIVE]"
                         ntu = "[INACTIVE]"
                    ...
                    if adwin[name].detected_change():
                         ...
                         variance_det = round(variance_det, 2)
                    ...
                    if((name == 'RESP' or name == 'HR' or name == 'SpO2' or name == 'PULSE')and(int(val1)>0)):
